...
@section('content')
    ...
    <h1 class="text-2xl font-bold mb-6">إضافة بطاقة جديدة</h1>

    <form method="POST" action="{{ route('decks.cards.store', $deck) }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-slate-200 p-6">
        @csrf
        @include('cards._form', ['category' => $category])
